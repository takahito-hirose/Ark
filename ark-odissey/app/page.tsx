...
/**
 * 🚢 PROJECT ODISSEY - ULTIMATE HUD LAYER (Emergency & Rich Mode Ready)
 * エマージェンシー演出、ECO/RICH切り替え、ログのセルフヒーリングまで
 * すべてを統合した最強の操舵室よ！💋
 */
      ...
      <div className="fixed inset-0 z-20 p-8 flex flex-col justify-between pointer-events-none">
        ...
        <div className="flex justify-between items-start pointer-events-auto">
          <motion.div 
            ...
            className={`bg-[#0f172a]/40 backdrop-blur-md border p-5 rounded-sm transition-colors duration-500 min-w-[280px] ${hasError ? 'border-[#ff5555]/50 shadow-[0_0_30px_rgba(255,85,85,0.2)]' : 'border-[#8be9fd]/30 shadow-[0_0_20px_rgba(139,233,253,0.1)]'}`}
          >
              ...
              <div className="mt-4 pt-3 border-t border-gray-800 flex items-center justify-between">
                ...
                <div className="flex bg-black/50 p-1 rounded-sm border border-gray-800">
                  <button 
                    onClick={() => setMode('ECO')}
                    disabled={isThinking}
                    className={`px-4 py-1 text-[10px] font-bold tracking-widest transition-all rounded-sm ${mode === 'ECO' ? 'bg-[#50fa7b] text-black shadow-[0_0_10px_rgba(80,250,123,0.5)]' : 'text-gray-500 hover:text-white'}`}
                  >
                    ECO
                  </button>
                  <button 
                    ...
                    disabled={isThinking}
                    className={`px-4 py-1 text-[10px] font-bold tracking-widest transition-all rounded-sm ${mode === 'RICH' ? 'bg-[#ffb86c] text-black shadow-[0_0_10px_rgba(255,184,108,0.5)]' : 'text-gray-500 hover:text-white'}`}
                  >
                    RICH
                  </button>
                </div>
          ...
          >
            <div className={`bg-[#0f172a]/40 backdrop-blur-md border px-6 py-3 rounded-sm flex items-center gap-4 transition-colors duration-500 ${hasError ? 'border-[#ff5555]/50' : 'border-[#ff79c6]/30'}`}>
              ...
              <div className={`w-10 h-10 rounded-full border flex items-center justify-center ${hasError ? 'border-[#ff5555]/50 text-[#ff5555]' : 'border-[#ff79c6]/50 text-[#ff79c6]'}`}>
                {hasError ? (
        ...
        <div className="flex gap-8 h-[40vh] pointer-events-auto items-stretch">
          ...
          <div className={`flex-1 h-full bg-[#0f172a]/40 backdrop-blur-md border rounded-sm flex flex-col overflow-hidden relative shadow-[0_0_40px_rgba(0,0,0,0.5)] transition-colors duration-500 ${hasError ? 'border-[#ff5555]/50' : 'border-[#bd93f9]/40'}`}>
            <div className={`p-3 px-5 border-b flex justify-between items-center transition-colors duration-500 ${hasError ? 'bg-[#ff5555]/10 border-[#ff5555]/30' : 'bg-[#bd93f9]/10 border-[#bd93f9]/20'}`}>
              ...
              <div className={`text-[10px] font-mono ${hasError ? 'text-[#ff5555]' : 'text-[#bd93f9]/50'}`}>{hasError ? 'SYS_FAILURE' : 'NODE: ARK_ODISSEY_V5'}</div>
            </div>
            
            <div className="flex-1 overflow-y-auto p-6 space-y-3 custom-scrollbar">
              <AnimatePresence initial={false}>
                {logs.map((log, i) => {
                  // 💋 ノアぴのセルフヒーリング：バックエンドの誤判定をフロントで強制補正！
              ...
              <form onSubmit={handleCommandSubmit} className="space-y-4">
                <div className="relative">
                  ...
                  <textarea
                    ...
                    disabled={isThinking}
                    placeholder={hasError ? "System halted. Enter new directives to reboot..." : "Describe your requirements in detail..."}
                    className={`w-full bg-black/60 border text-white pl-8 pr-4 py-4 rounded-sm focus:outline-none focus:ring-1 transition-all disabled:opacity-50 font-mono text-[13px] resize-none h-32 custom-scrollbar
                ...
                <button 
                  ...
                  className={`w-full py-3 rounded-sm uppercase text-[11px] tracking-[0.3em] font-bold transition-all disabled:opacity-50 flex items-center justify-center gap-3
