...
ClineやCursorといった既存のツールが「最強の電動ドリル（高機能なエディタプラグイン）」であるなら、ARKは**「電動ドリルを持った大工の棟梁」**です。指示を出してコーヒーを飲んでいる間に、設計から材料調達、組み立て、掃除（PR作成）まで全自動で完遂します。

* **💥 「アシスタント」ではなく「自律型同僚」**: Fire and Forget（撃ちっ放し）。ブランチの作成からテスト、エラーの自己修復（Auto-Bailing）、GitHubへのPushまで、エディタを開くことなく全自動で進行します。
* **📚 使えば使うほど賢くなる「大図書館」**: プロジェクトの「掟」と過去のエラー解決の「経験」を蓄積。チーム特有の暗黙知を吸収し、プロジェクト専用の最強の職人へと成長します。
* **🔭 自己完結するリサーチ能力**: 未知のライブラリやエラーに直面しても、自律的にWeb検索（Brave Search）を行い、公式ドキュメントを読んで知識を補完します。
* **🪙 お財布をコントロールする「金庫番」**: タスクの難易度に応じて、無料のローカルLLM（Eco）と最高峰のクラウドLLM（Rich）を動的に切り替え。コストと品質を完全にコントロールできます。
